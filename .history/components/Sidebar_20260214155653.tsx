...
const Sidebar: React.FC<SidebarProps> = ({ role, activeTab, onTabChange, onLogout, onReportClick, isOpen, onToggle }) => {
  const menuItems = role === UserRole.ADMIN ? [
    ...
    { id: 'admin-upload', label: 'Upload', icon: Icons.Book },
    { id: 'admin-users', label: 'User Management', icon: Icons.User },
      ...
      <aside className={`w-64 bg-[#17A2B8] text-white h-screen fixed left-0 top-0 flex flex-col z-50 shadow-2xl transition-transform duration-300 transform border-r border-white/10 ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
        ...
        <div className="p-8 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Icons.ASTULogo className="w-12 h-12 rounded-lg object-cover bg-white" />
            <div className="font-black text-white text-xl leading-none tracking-tighter uppercase">
              ዘብ AI <br />
              <span className="text-white/80 text-xs tracking-widest">Smart Safety</span>
            </div>
          </div>
          <button
            onClick={() => onToggle?.(false)}
        ...
        <div className="flex-1 mt-8 px-4 space-y-2 overflow-y-auto">
          ...
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-6 py-5 rounded-xl transition-all font-extrabold text-lg ${activeTab === item.id
        ...
        <div className="p-6 border-t border-white/10 space-y-4">
          ...
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-3 px-4 py-3 text-red-300 hover:text-red-200 hover:bg-white/10 rounded-xl transition-colors font-bold text-sm"
          >
            <Icons.LogOut />
