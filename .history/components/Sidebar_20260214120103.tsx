    ...
    <>
      ...
      <aside className={`w-64 bg-[#17A2B8] text-white h-screen fixed left-0 top-0 flex flex-col z-50 shadow-2xl transition-transform duration-300 transform border-r border-white/10 ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
        ...
        <div className="flex-1 mt-8 px-4 space-y-2 overflow-y-auto">
          {role === UserRole.STUDENT && (
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-4 rounded-xl bg-white text-[#17A2B8] shadow-lg hover:shadow-white/20 transition-all active:scale-95 mb-6 font-bold text-sm"
            >
              <Icons.Shield />
        ...
        <div className="p-6 border-t border-white/10 space-y-4">
          ...
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-3 px-4 py-3 text-white/90 hover:bg-white/10 rounded-xl transition-colors font-bold text-sm"
          >
            <Icons.LogOut />
