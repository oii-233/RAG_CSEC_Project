...
const Sidebar: React.FC<SidebarProps> = ({ role, activeTab, onTabChange, onLogout, onReportClick, isOpen, onToggle }) => {
  ...
  return (
    <>
      ...
      <aside className={`w-64 bg-white text-[#0F2A3D] h-screen fixed left-0 top-0 flex flex-col z-50 shadow-2xl transition-transform duration-300 transform border-r border-gray-100 ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
        }`}>
        <div className="p-8 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Icons.ASTULogo className="w-8 h-8 rounded-lg object-cover" />
            <div className="font-black text-[#0F2A3D] text-sm leading-none tracking-tighter uppercase">
              ዘብ AI <br />
