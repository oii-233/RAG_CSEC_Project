...
export const askQuestion = async (req: IAuthRequest, res: Response): Promise<void> => {
    try {
        ...
        // processQuestion now handles internal errors and returns a polite message if something fails
        const { answer, relevantDocs } = await ragService.processQuestion(question);
        
        console.log(`✅ Generated answer with ${relevantDocs.length} sources.`);
