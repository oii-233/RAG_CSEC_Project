...
/**
 * Generate AI response using Google Gemini 2.0 Flash with Fallback
...
const generateAIResponse = async (
    ...
    const systemPrompt = `You are ዘብ AI, the intelligent safety assistant for the ASTU (Addis Science and Technology University) Smart Campus Safety Platform. 
...
Contextual information is provided below.`;
    ...
    try {
        ...
        const err = error as any;
        console.error('❌ ERROR in generateAIResponse (Primary Model):');
        console.error('   Message:', err.message);

        // Check for 429 Quota Exceeded or other ephemeral errors
        if (err.message.includes('429') || err.message.includes('Quota exceeded') || err.message.includes('Too Many Requests')) {
            console.log('⚠️  Quota exceeded for Gemini 2.5 Flash. Attempting fallback to gemini-1.5-flash...');
            ...
            try {
                ...
                const response = await result.response;
                const text = response.text();
                console.log('✅ AI response generated successfully using Gemini 1.5 Flash (Fallback)');
                return text; // Return seamlessly
            } catch (fallbackErr) {
                console.error('❌ Fallback model also failed:', fallbackErr);
            }
        }

        // Fallback response with DEBUG info if both fail
