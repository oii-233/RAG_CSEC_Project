...
export const askQuestion = async (req: IAuthRequest, res: Response): Promise<void> => {
    try {
        ...
        // Note: processQuestion handles its own fallbacks but might throw if critical services are missing
        let answer: string;
        let relevantDocs: any[];

        try {
            const ragResponse = await ragService.processQuestion(question);
            answer = ragResponse.answer;
            relevantDocs = ragResponse.relevantDocs;
        } catch (error) {
            console.error('⚠️ RAG Service Failed:', error);
            answer = "I apologize, but I'm having trouble connecting to my knowledge base right now. Please try again later.";
            relevantDocs = [];
        }
