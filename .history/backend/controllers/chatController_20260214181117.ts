...
export const askQuestion = async (req: IAuthRequest, res: Response): Promise<void> => {
    try {
        ...
        // Wrapped in try/catch to expose specific service errors to the client in dev mode
        let responseData;
        try {
             responseData = await ragService.processQuestion(question);
        } catch (serviceError) {
             console.error("❌ RAG Service Error:", serviceError);
             const errorMessage = serviceError instanceof Error ? serviceError.message : "Unknown RAG Error";
             
             // Check for specific error types to give better feedback
             let statusCode = 502;
             if (errorMessage.includes("API Key")) statusCode = 500;
             if (errorMessage.includes("embedding")) statusCode = 503;

             res.status(statusCode).json({
                 success: false,
                 message: 'Failed to process question via AI service.',
                 error: errorMessage, // Always return error for now to help user debug
                 stack: process.env.NODE_ENV === 'development' && serviceError instanceof Error ? serviceError.stack : undefined
             });
             return;
        }

        const { answer, relevantDocs } = responseData;
