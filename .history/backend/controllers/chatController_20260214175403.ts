...
export const askQuestion = async (req: IAuthRequest, res: Response): Promise<void> => {
    try {
        ...
        if (!question || typeof question !== 'string' || question.trim().length === 0) {
            res.status(400).json({ success: false, message: 'Please provide a valid question' });
            return;
        }

        console.log(`💬 Processing question from User ${req.user.id}: "${question.substring(0, 50)}..."`);

        // 2. Process with RAG Service
