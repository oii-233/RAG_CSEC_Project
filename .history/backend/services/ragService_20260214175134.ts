...
`;
            ...
            // Fallback for model not found or other errors
            if (error.message?.includes('404') || error.message?.includes('not found')) {
                 console.error('⚠️ Model not found. Attempting fallback to gemini-1.5-flash...');
                 try {
                    const fallbackModel = this.genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });
                    // ... (simplified fallback logic if needed, or just return error)
                 } catch (e) {}
            }
