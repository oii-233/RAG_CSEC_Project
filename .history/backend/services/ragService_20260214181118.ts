    ...
    async generateEmbedding(text: string): Promise<number[] | null> {
        try {
            if (!this.voyageApiKey) {
                console.error("❌ Voyage API Key is missing. Check VOYAGE_API_KEY in .env");
            ...
            console.error("❌ Voyage API returned no embedding data", response.data);
            return null;
        } catch (error) {
            if (axios.isAxiosError(error)) {
                console.error('❌ Voyage API Error:', error.response?.data || error.message);
                throw new Error(`Voyage API Error: ${JSON.stringify(error.response?.data || error.message)}`);
            }
